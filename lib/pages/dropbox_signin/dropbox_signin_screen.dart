...
class DropboxAuthentication extends StatefulWidget {
  ...
  DropboxAuthentication({
    required this.selectedUrl, required this.oldContext
  });
...
//    print("Storing token");
  ...
  selectingCalibreLibrary(key, val, update, token) {
    ...
    MetadataCacher().downloadAndCacheMetadata(token:token, path: '').then((val) {
      if (val == 1) {
//        print("storing token");
                            ...
                            return WillPopScope(
                              onWillPop: () async {
                                ...
                                await MetadataCacher()
                                    .downloadAndCacheMetadata(token:token, path: '')
                                    .then((val) {
                                  if (val == 1) {
                                    storeStringInSharedPrefs('token', token);
                                    update.changeTokenState(true);
...
//                          print("stored token");
                    ...
                    Scaffold.of(widget.oldContext).showSnackBar(
