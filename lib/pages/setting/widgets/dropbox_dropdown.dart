...
import 'package:calibre_carte/pages/setting/widgets/connect_button.dart';
import 'package:calibre_carte/pages/setting/widgets/logout_button.dart';
import 'package:calibre_carte/pages/setting/widgets/refresh_button.dart';
...
class _DropboxDropdownState extends State<DropboxDropdown> {
  ...
  late String dropboxEmail;
  late String selected_calibre_lib_dir;
  late String selected_lib_name;
  late String token;
  List<String> dirNames = [];
  late int noOfCalibreLibs;
  late bool hasChanged;
  late String clientId;
  late Configuration config;
  late StreamSubscription _sub;
  static const String NO_CODE_URI = 'no_code_uri';
...
//    print(token);
    ...
    try {
      Response response = await post(url as Uri, headers: headers, body: json);
...
//    print(code);
    ...
    Response response = await post(url as Uri, body: json);
    int statusCode = response.statusCode;
    String body = response.body;
    return response;
  }
                              ...
                              child: Container(
                                ...
                                child: Column(
                                  children: <Widget>[
                                    Row(
                                      children: <Widget>[
                                        Icon(Icons.folder, color: colorTheme.settingsIcon,),
                                        const SizedBox(width: 10,),
                                        Text("Change Library",
                                            style: TextStyle(
                                                color: colorTheme.headerText, fontFamily: 'Montserrat', fontSize: 15))
                                      ],
                                    ),
                                  ],
                                ),
                                  ...
                                  if (snapshot.connectionState ==
                                    ...
                                    return Column(
                                      children: <Widget>[
                                        Center(
                                          child: Text(
                                            'Loading...', style: TextStyle(color: colorTheme.headerText),
                                          ),
                                        ),
                                        const SizedBox(height: 7,),
                                      ],
